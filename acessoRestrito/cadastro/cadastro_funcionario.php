...
<body>
    ...
    <script>
        // Interação Médico
        // Interação Médico
        const vinc = document.querySelector('#vinculo'); // Seleciona o elemento HTML com o ID 'vinculo', que provavelmente é um campo de seleção (select).

        vinc.onchange = (e) => { // Adiciona um evento de mudança (change) ao elemento 'vinculo'.
            let els = document.querySelectorAll('.medico'); // Seleciona todos os elementos com a classe 'medico'.

            if(vinc.value == 'M'){ // Verifica se o valor do campo 'vinculo' é igual a 'M' (possivelmente indicando um médico).
                for(i = 0; i < els.length; i++){ // Loop pelos elementos selecionados.
                    els[i].style.display = 'block'; // Define o estilo de exibição para 'block', tornando-os visíveis.
                    els[i].querySelector('.form-control').setAttribute('required', true); // Adiciona o atributo 'required' aos campos com a classe 'form-control'.
                }
            } else { // Se o valor do campo 'vinculo' não for igual a 'M'.
                for(i = 0; i < els.length; i++){ // Loop pelos elementos selecionados.
                    els[i].style.display = 'none'; // Oculta os elementos.
                    els[i].querySelector('.form-control').setAttribute('required', false); // Remove o atributo 'required' dos campos com a classe 'form-control'.
                }
            }
        }

        // Envio Formulário
        const form = document.querySelector('#formCad'); // Seleciona o formulário com o ID 'formCad'.

        form.onsubmit = (e) => { // Adiciona um evento de envio (submit) ao formulário.
            e.preventDefault(); // Impede o comportamento padrão do envio do formulário.

            alert('Funcionário cadastrado com sucesso!'); // Exibe um alerta com a mensagem 'Funcionário cadastrado com sucesso!'.
            window.location.assign('../'); // Redireciona o usuário para o diretório pai do local atual.
        }
